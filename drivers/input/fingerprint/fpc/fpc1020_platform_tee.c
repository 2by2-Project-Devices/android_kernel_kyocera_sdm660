				 ...
				 unsigned long event, void *data)
{
	...
	unsigned int last_fb_state = fpc1020->fb_state;
	...
	if (*blank <= FB_BLANK_NORMAL) {
		if (event == FB_EVENT_BLANK) {
			dev_info(fpc1020->dev, "%s: UNBLANK\n", __func__);
		}
		else if (event == FB_EARLY_EVENT_BLANK || event == FB_R_EARLY_EVENT_BLANK) {
			dev_info(fpc1020->dev, "%s: EARLY UNBLANK\n", __func__);
		}
		fpc1020->fb_state = 0;
	} else if (*blank == FB_BLANK_POWERDOWN) {
		if (event == FB_EVENT_BLANK) {
			dev_info(fpc1020->dev, "%s: POWERDOWN\n", __func__);
		}
		else if (event == FB_EARLY_EVENT_BLANK || event == FB_R_EARLY_EVENT_BLANK) {
			dev_info(fpc1020->dev, "%s: EARLY POWERDOWN\n", __func__);
		}
		fpc1020->fb_state = 1;
	}
...
static void fpc1020_shutdown(struct platform_device *pdev)
{
	struct fpc1020_data *fpc1020 = platform_get_drvdata(pdev);

	if (fpc1020 && fpc1020->fingerprint_pinctrl) {
		if (atomic_read(&fpc1020->drop_det_available)) {
			atomic_set(&fpc1020->drop_det_available, 0);
			if (atomic_read(&fpc1020->drop_det_en)) {
				atomic_set(&fpc1020->drop_det_en, 0);
