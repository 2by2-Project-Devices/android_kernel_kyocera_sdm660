...
MODULE_FIRMWARE(CY01_FW_FILE_NAME);
MODULE_FIRMWARE(CY01_CFG_FILE_NAME);
MODULE_FIRMWARE(CY02_FW_FILE_NAME);
MODULE_FIRMWARE(CY02_CFG_FILE_NAME);
MODULE_FIRMWARE(CY03_FW_FILE_NAME);
MODULE_FIRMWARE(CY03_CFG_FILE_NAME);
MODULE_FIRMWARE(CY04_FW_FILE_NAME);
MODULE_FIRMWARE(CY04_CFG_FILE_NAME);
...
#define CY_GLOVE_OFF  0
#define CY_GLOVE_ON   1
#define CY_GLOVE_AUTO 2
...
static int cyttsp5_hid_output_enter_lpwge_state_(
		struct cyttsp5_core_data *cd, u8 data, u8 *return_data)
{
	int write_length = 0;
	u8 param[1] = { data };
	int rc;
	struct cyttsp5_hid_output hid_output = {
		HID_OUTPUT_APP_COMMAND(HID_OUTPUT_SUSPEND_ENTER_LPWG),
		.write_length = write_length,
		.write_buf = param,
	};
...
static int cyttsp5_set_touchmode_enabled(struct cyttsp5_core_data *cd, u8 mode)
{
	...
	if( cd->glove_mode == CY_GLOVE_OFF &&
		mode == CY_TME_FINGER_AND_GLOVE){
		dev_info(cd->dev, "%s: Glove is invalidated\n", __func__);
		return 0;
	}

	if( cd->glove_mode == CY_GLOVE_ON &&
		mode == CY_TME_FINGER_ONLY){
		dev_info(cd->dev, "%s: Glove mode only\n", __func__);
		return 0;
	}
...
static int cyttsp5_put_device_into_lpwg_(struct cyttsp5_core_data *cd)
{
	int rc;
	u8 status = 0;
	...
	rc = cyttsp5_hid_output_enter_lpwge_state_(cd,
			cd->easy_wakeup_gesture, &status);
	if (rc || status == 0)
		return -EBUSY;
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	case KC_TOUCH_EVENT_RESTART:
		dev_dbg(dev, "%s: KC Event Restart (cd=%p)\n", __func__, cd);
	...
	case KC_TOUCH_EVENT_STOP:
		dev_dbg(dev, "%s: KC Event Stop (cd=%p)\n", __func__, cd);
		mutex_lock(&md->mt_lock);
...
#ifdef CONFIG_TOUCHSCREEN_KC_TOUCHMODE_IOCTL
static ssize_t cyttsp5_touchmode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
...
static ssize_t cyttsp5_touchmode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	unsigned long value;
	...
	if (retval < 0) {
		dev_err(dev, "%s: Invalid value\n", __func__);
		goto cyttsp5_touchmode_store_exit;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(buf, CY_MAX_PRBUF_SIZE, "%x\n", cd->glove_mode);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if ( cd->glove_mode != CY_GLOVE_OFF &&
		 cd->glove_mode != CY_GLOVE_ON  &&
		 cd->glove_mode != CY_GLOVE_AUTO){
		pr_err("%s: Error: invalid value\n",__func__);
		cd->glove_mode = CY_GLOVE_AUTO;
		return -EINVAL;
	}
...
static struct device_attribute attributes[] = {
	...
	__ATTR(touch_mode, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP,
			cyttsp5_touchmode_show, cyttsp5_touchmode_store),
#endif
	__ATTR(glove_mode, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP,
		 cyttsp5_glove_mode_show, cyttsp5_glove_mode_store),
};
...
static int cyttsp5_eco_and_suspend(struct cyttsp5_core_data *cd)
{
	...
	rc = cyttsp5_tp_i2c_pinctrl_select(cd->dev, 2);
	if(rc){
		pr_err("%s I2C_TP set Fail.\n", __func__);
	}

	rc = cyttsp5_pinctrl_select(cd, false);
	if(rc){
...
#ifndef CONFIG_INCELL_TOUCH
	gpio_set_value(cd->cpdata->vdda_gpio, 0);
#endif
...
{
	int rc;
	u8 sensing_mode =CY_IB_SM_MUTCAP | CY_IB_SM_SELFCAP;
	u8 staus;
...
#ifndef CONFIG_INCELL_TOUCH
	gpio_set_value(cd->cpdata->vdda_gpio, 1);

	if(!gpio_get_value(cd->cpdata->vccd_gpio))
		gpio_set_value(cd->cpdata->vccd_gpio, 1);
...
void cyttsp5_display_set_easywake_mode(void *cyttsp5_data, int easywake_mode){
	struct cyttsp5_core_data *cd;
...
int cyttsp5_display_suspend(void *cyttsp5_data, int order){
	...
	if(order == 1){
		...
		if(cd->easy_wakeup_gesture == 1)
			cyttsp5_put_device_into_lpwg_(cd);
		else
...
int cyttsp5_shutdown(struct cyttsp5_core_data *cd)
{
	...
	gpio_set_value(cd->cpdata->vdda_gpio, 0);
	gpio_set_value(cd->cpdata->vccd_gpio, 0);
