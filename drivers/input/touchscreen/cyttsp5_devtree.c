		...
		struct device_node *core_node)
{
	...
	pr_info("%s: cyttsp5: rst value = %d\n",__func__,value);
	pdata->rst_gpio = value;

	value = of_get_named_gpio(core_node, "cy,vdda_gpio", 0);
	if (gpio_is_valid(value))
		pdata->vdda_gpio = value;

	value = of_get_named_gpio(core_node, "cy,vccd_gpio", 0);
	if (gpio_is_valid(value))
		pdata->vccd_gpio = value;
	...
	rc = of_property_read_u32(core_node, "cy,glove_valid", &value);
	if (!rc)
		pdata->glove_valid = (u8)value;

	rc = of_property_read_u32(core_node, "cy,fw_type", &value);
	if (!rc)
