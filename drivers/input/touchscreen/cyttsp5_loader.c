		...
		u32 fw_ver_new, u32 fw_revctrl_new)
{
	...
	if (fw_revctrl_img == 0x000DCD29) {
		dev_dbg(dev, "%s: TP is older, will NOT upgrade\n",
				__func__);
		return -1;
	}
...
static int cyttsp5_ldr_exit_(struct device *dev)
{
	...
	cd->ldr_exit_flg = 1;
	rc = cmd->nonhid_cmd->launch_app(dev, 0);
	cd->ldr_exit_flg = 0;
		...
		const struct firmware *fw)
{
	...
	if (upgrade > 0)
		...
		if((ld->si->cydata.fw_ver_conf >> 8) != 0x24){
			pr_err("%s ver_conf = %x\n",__func__, (ld->si->cydata.fw_ver_conf>>8));
			return 1;
		}
	}
		...
		void *context)
{
	...
	if (upgrade) {
		...
		complete_all(&cd->fw_update_done);
		return;
	}
...
#define FILENAME_LEN_MAX 64
	...
	if (cd->cpdata->fw_type == 1)
		snprintf(filename, FILENAME_LEN_MAX, "%s", CY01_FW_FILE_NAME);
	else if (cd->cpdata->fw_type == 2)
		snprintf(filename, FILENAME_LEN_MAX, "%s", CY02_FW_FILE_NAME);
	else if (cd->cpdata->fw_type == 3)
		snprintf(filename, FILENAME_LEN_MAX, "%s", CY03_FW_FILE_NAME);
	else if (cd->cpdata->fw_type == 4)
		snprintf(filename, FILENAME_LEN_MAX, "%s", CY04_FW_FILE_NAME);
	else {
		...
		void *context)
{
	...
	cfg_ver_new = (ld->config_data[15] << 8) + (ld->config_data[16]);
...
static int upgrade_ttconfig_from_builtin(struct device *dev)
{
	...
	if (cd->cpdata->fw_type == 1){
		dev_info(dev, "%s: Filename: %s\n", __func__, CY01_CFG_FILE_NAME);
		retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				CY01_CFG_FILE_NAME, dev, GFP_KERNEL, dev,
				_cyttsp5_ttconfig_cont_builtin);
	} else if (cd->cpdata->fw_type == 2){
		dev_info(dev, "%s: Filename: %s\n", __func__, CY02_CFG_FILE_NAME);
		retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				CY02_CFG_FILE_NAME, dev, GFP_KERNEL, dev,
				_cyttsp5_ttconfig_cont_builtin);
	} else if (cd->cpdata->fw_type == 3){
		dev_info(dev, "%s: Filename: %s\n", __func__, CY03_CFG_FILE_NAME);
		retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				CY03_CFG_FILE_NAME, dev, GFP_KERNEL, dev,
				_cyttsp5_ttconfig_cont_builtin);
	} else if (cd->cpdata->fw_type == 4){
		dev_info(dev, "%s: Filename: %s\n", __func__, CY04_CFG_FILE_NAME);
		retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				CY04_CFG_FILE_NAME, dev, GFP_KERNEL, dev,
				_cyttsp5_ttconfig_cont_builtin);
	} else {
		dev_err(dev, "%s: No Firmware\n", __func__);
		goto exit;
