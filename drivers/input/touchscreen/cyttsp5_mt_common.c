...
//		md->mt_function.input_sync(md->input);
	...
	input_report_abs(md->input,ABS_MT_POSITION_X,500);
	input_report_abs(md->input,ABS_MT_POSITION_Y,500);
	...
	input_report_abs(md->input,ABS_MT_POSITION_X,600);
	...
	if (md->mt_function.final_sync)
		md->mt_function.final_sync(md->input, 1, 0, &ids);
...
static int cyttsp5_mt_wake_attention(struct device *dev)
{
	...
	int state;
	state = atomic_read(&cd->oem_state);
	
	if((state & OEM_STATE_RESUME_BIT) != 0){
		return 0;
	}
