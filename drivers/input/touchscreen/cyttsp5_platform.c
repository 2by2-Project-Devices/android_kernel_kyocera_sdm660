...
*	rc |= cyttsp5_tp_i2c_pinctrl_select(dev, 0);
*	usleep_range(1000,5000);
*	if (rc)
*		pr_err("%s pin select false is Error.\n", __func__);
*
*	gpio_set_value(pdata->ldo_gpio, 0);
*	rc |= regulator_disable(cd->sensor_vdd);
*	msleep(20);
*
*	rc |= regulator_enable(cd->sensor_vdd);
*	gpio_set_value(pdata->ldo_gpio, 1);
*	usleep_range(1000,5000);
*	if (rc)
*		pr_err("%s Regulator control Error.\n", __func__);
*
*	rc |= cyttsp5_pinctrl_select(cd, true);
*	rc |= cyttsp5_tp_i2c_pinctrl_select(dev, 1);
*	usleep_range(4000,5000);
*	if (rc)
*		pr_err("%s pin select true is Error.\n", __func__);
*/
	gpio_set_value(rst_gpio, 1);
	msleep(20);
	dev_dbg(dev,
		...
		int on, struct device *dev)
{
	...
	if (on) {
		rc = gpio_request(rst_gpio, "cyttsp5_rst");
		if (rc < 0) {
			gpio_free(rst_gpio);
			rc = gpio_request(rst_gpio, "cyttsp5_rst");
		}
		if (rc < 0) {
			...
			if (rc < 0) {
				...
				rc = gpio_request(irq_gpio, "cyttsp5_irq");
				if (rc < 0) {
					gpio_free(irq_gpio);
