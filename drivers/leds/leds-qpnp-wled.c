...
#ifdef LED_DEBUG
#define LED_V_LOG(msg, ...)	\
	pr_notice("[LEDDRV][%s][V](%d): " msg "\n", __func__, __LINE__, ## __VA_ARGS__)
#define LED_D_LOG(msg, ...)	\
...
#define LED_E_LOG(msg, ...)	\
	pr_err   ("[LEDDRV][%s][E](%d): " msg "\n", __func__, __LINE__, ## __VA_ARGS__)
#define LED_N_LOG(msg, ...)	\
...
#define QPNP_WLED_MODULE_EN_VALUE	0x80
...
static struct qpnp_wled *g_wled = NULL;
...
#ifdef CONFIG_LEDS_OEM
	if (level > LED_FULL) {
		pr_err("level(%d) is invalid value. set to %d\n", level, LED_FULL);
		level = LED_FULL;
	}
	KCLIGHT_BRIGHT_TO_BL(level, wled->cdev.brightness, WLED_MAX_LEVEL_4095, LED_FULL);
	pr_debug("[LIGHT]qpnp_wled_set_level level=%d brightness=%d\n", level, wled->cdev.brightness);
#endif
...
static void qpnp_wled_work(struct work_struct *work)
{
	...
	if (!light_led_disp_enabled()) {
		LED_N_LOG("discard light_led_disp_enabled()");
		return;
	}
	...
	if (wled->brt_map_table) {
		...
		if( !light_lcd_power_enabled() ){
			LED_E_LOG("discard light_lcd_power_enabled()");
			level = 0;
		} else {
...
#ifndef CONFIG_LEDS_OEM
	wled->cdev.max_brightness = WLED_MAX_LEVEL_4095;
#else
	wled->cdev.max_brightness = LED_FULL;

	wled->cdev.usr_brightness_req = 38;
	wled->cdev.brightness = 38;

	rc = qpnp_wled_read_reg(wled,
		QPNP_WLED_MODULE_EN_REG(wled->ctrl_base), &reg);
	if (rc < 0){
		pr_err("fail to read QPNP_WLED_MODULE_EN_REG state \n");
	}else{
		reg &= QPNP_WLED_MODULE_EN_VALUE;
		if(reg){
			wled->prev_state = 1;
			pr_debug("wled->prev_state = %d ",wled->prev_state);
		}
	}

	pr_debug("brightness:%d max_brightness:%d usr_brightness_req:%d\n",
		wled->cdev.brightness, wled->cdev.max_brightness, wled->cdev.usr_brightness_req);
#endif
...
{
	LED_V_LOG("[IN] panel_class=0x%x", panel_class);
	switch( panel_class ){
	case LIGHT_LCD_PANEL0:
		LED_V_LOG("panel class = LIGHT_LCD_PANEL0");
		break;
	default:
		LED_E_LOG("unknown panel class");
		break;
	}
...
static void light_led_schedule_wled_work(void)
{
	if (g_wled) {
		if (g_wled->cdev.brightness) {
			schedule_work(&g_wled->work);
		}
	}
}
...
int32_t light_led_disp_set(e_light_main_wled_disp disp_status)
{
	...
	LED_V_LOG("[IN] disp_status=[0x%x]", (uint32_t)disp_status);

	if ((atomic_read(&g_display_detect)) != 0) {
		LED_V_LOG("already determined.");
		return ret;
	}

	switch(disp_status) {
	case LIGHT_MAIN_WLED_LCD_EN:
		LED_N_LOG("LIGHT_MAIN_WLED_LCD_EN");
		atomic_set(&g_display_detect,1);
		light_led_schedule_wled_work();
		break;
	case LIGHT_MAIN_WLED_LCD_DIS:
		LED_N_LOG("LIGHT_MAIN_WLED_LCD_DIS");
		atomic_set(&g_display_detect,-1);
		break;
	default:
		break;
...
int32_t light_led_disp_power_set(e_light_main_wled_disp disp_status)
{
	...
	case LIGHT_MAIN_WLED_LCD_PWREN:
		LED_N_LOG("LIGHT_MAIN_WLED_LCD_PWREN");
	...
	case LIGHT_MAIN_WLED_LCD_PWRDIS:
		LED_N_LOG("LIGHT_MAIN_WLED_LCD_PWRDIS");
		atomic_set(&g_detect_lcd_pwren,-1);
		if (light_led_disp_enabled()) {
			light_led_schedule_wled_work();
		}
...
int32_t light_led_disp_power_set(e_light_main_wled_disp disp_status)
{
	LED_N_LOG("invalid DISCARD_BL_LCDPOWER_OFF disp_status=0x%x",  (uint32_t)disp_status);
	return 0;
}
EXPORT_SYMBOL(light_led_disp_power_set);
