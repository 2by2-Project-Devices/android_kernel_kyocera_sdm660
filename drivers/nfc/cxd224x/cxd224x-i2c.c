...
/* wake lock for HOSTINT */
#define CXD224X_WAKE_LOCK_NAME "cxd224x-i2c"
/* wake lock timeout for low-power-mode (sec) */
...
static void cxd224x_workqueue(struct work_struct *work)
{
	struct cxd224x_dev *cxd224x_dev =
		container_of(work, struct cxd224x_dev, qmsg);
	unsigned long flags;
...
static int cxd224x_dev_open(struct inode *inode, struct file *filp)
{
	int ret = 0;
	int call_enable = 0;
	struct cxd224x_dev *cxd224x_dev = container_of(
		filp->private_data, struct cxd224x_dev, cxd224x_device);
	...
	dev_info(&cxd224x_dev->client->dev, "open %d,%d users=%d\n",
...
static int cxd224x_dev_release(struct inode *inode, struct file *filp)
{
	...
	dev_info(&cxd224x_dev->client->dev, "release %d,%d users=%d\n",
		 imajor(inode), iminor(inode), cxd224x_dev->users);
				       ...
				       unsigned long arg)
{
	...
	case CXDNFC_RST_CTL:
		...
		dev_info(&cxd224x_dev->client->dev, "%s, rst arg=%d\n",
			 __func__, (int)arg);
		return (queue_work(cxd224x_dev->wqueue, &cxd224x_dev->qmsg)
				? 0
				: 1);
#endif
		break;
			    ...
			    struct cxd224x_platform_data *pdata)
{
	...
	/*nfc_int*/
	pdata->irq_gpio =
		of_get_named_gpio_flags(dev->of_node, "sony,nfc_int", 0, NULL);
	if (pdata->irq_gpio < 0) {
...
#if defined(CONFIG_NFC_CXD224X_VEN) || defined(CONFIG_NFC_CXD224X_VEN_MODULE)
	pdata->en_gpio =
		of_get_named_gpio_flags(dev->of_node, "sony,nfc_ven", 0, NULL);
	if (pdata->en_gpio < 0) {
...
#if defined(CONFIG_NFC_CXD224X_RST) || defined(CONFIG_NFC_CXD224X_RST_MODULE)
	ret = gpio_request_one(platform_data->rst_gpio, GPIOF_OUT_INIT_HIGH,
			       "nfc_rst");
	if (ret)
		goto err_exit;
	rst_gpio_ok = 1;
...
#ifdef CONFIG_OF
static const struct of_device_id cxd224x_dt_match[] = {
...
static struct i2c_driver cxd224x_driver = {
	...
	.driver = {
		...
		.of_match_table = of_match_ptr(cxd224x_dt_match),
#ifdef CONFIG_PM
			.pm = &cxd224x_pm_ops,
#endif
		},
