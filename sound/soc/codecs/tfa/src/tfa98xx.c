...
void tfa_set_calibration_flag(struct snd_soc_codec *codec)
{
	struct tfa98xx *tfa98xx = snd_soc_codec_get_drvdata(codec);
	tfaSetAcsAndCalibrationFlag(tfa98xx->handle);
}

int tfa98xx_mtpex_get(struct snd_soc_codec *codec, unsigned int *val)
{
	struct tfa98xx *tfa98xx = snd_soc_codec_get_drvdata(codec);
	enum Tfa98xx_Error err, status;
	unsigned short value;
...
#ifdef CONFIG_KYOCERA_MSND
extern int kaudio_codec_init(struct snd_soc_codec *codec);
#endif
